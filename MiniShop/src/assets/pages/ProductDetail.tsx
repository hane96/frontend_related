...
function ProductDetail() {
  ...
  if (error) {
    return (
      <p className="text-[#E66F55] bg-[#FFF7F0] h-screen">Error: {error}</p>
    );
  }
  ...
  if (product) {
    return (
      <div className="bg-white text-[#5A3E36] min-h-screen flex gap-8 md:gap-12 flex-col md:flex-row p-8">
        <div className="md:w-[50vw] flex items-center justify-center">
          <img
            src={product.image}
            alt={product.title}
            className="block max-h-[300px] md:max-h-[600px] w-auto object-contain hover:scale-105"
          />
        </div>
        ...
        <div className="flex flex-col text-left md:text-center gap-4 mt-8 md:w-[50vw]">
          ...
          <p className="leading-relaxed text-[#7F5A4F]">
            {product.description}
          </p>
          ...
          {localStorage.getItem("login") == "true" ? (
            <>
              <div className="border-[#FFCCBC] flex items-center gap-4 border rounded px-4 py-2 self-center">
                ...
                >
                  目前數量:{" "}
                  {cart.find((item) => item.productId == id_num)?.num || 0}
                </motion.span>
                <button
                  onClick={addToCart}
                  className="border border-[#FFCCBC] p-2 md:p-1 rounded hover:bg-[#FFF1EB] cursor-pointer"
  ...
  return (
    <div
      className="flex flex-col justify-center text-center h-screen"
